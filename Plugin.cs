...
namespace MycopunkUpgradeSorting
{
    public class FilterPanelUI
    {
        ...
        {
            var statLabel = CreateUITextObject("StatFilterLabel", "Show Only Upgrades With:", 12, filterPanel.transform);
            ...
            foreach (var propertyName in contextProperties.OrderBy(p => p))
            {
                ...
                var statToggle = CreateUIToggleObject($"StatToggle_{propertyName}", displayName, 8, contentGo.transform,
                    (value) => {
                        if (value) {
        ...
        private void RefreshUpgrades()
        {
            ...
            try {
                ...
                AccessTools.Method(typeof(GearDetailsWindow), "SortUpgrades", new Type[] { typeof(GearDetailsWindow.SortingMethod), typeof(bool) })?.Invoke(window, new object[] { currentMethod, false });
        ...
        public void Toggle()
        {
            ...
            var rectTransform = filterPanel.GetComponent<RectTransform>();
            rectTransform.anchoredPosition = isExpanded ?
                new Vector2(-190, -80) : new Vector2(-200, -80);
        }
            ...
            try
            {
                ...
                button.onClick.AddListener(() => {
                    ...
                    if (filterPanel != null)
                    {
                        foreach (Transform child in filterPanel.transform)
                        {
                            if (child.name.StartsWith("FilterButton_"))
                            {
                                ...
                                if (buttonImage != null && child.GetComponentInChildren<TextMeshProUGUI>()?.text != null)
                                {
                                    buttonImage.color = new Color(0.3f, 0.3f, 0.3f, 1f);
                                }
                            }
                    ...
                    if (filterPanel != null)
                    {
                        ...
                        if (showAllBtn != null) showAllBtn.GetComponent<Image>().color = new Color(0.5f, 0.7f, 0.5f, 1f);
                        if (onlyFavBtn != null) onlyFavBtn.GetComponent<Image>().color = new Color(0.3f, 0.3f, 0.3f, 1f);
                        if (hideFavBtn != null) hideFavBtn.GetComponent<Image>().color = new Color(0.3f, 0.3f, 0.3f, 1f);
                    }
...
namespace MycopunkUpgradeSorting
{
    ...
    public class UpgradeSortingPlugin : BaseUnityPlugin
    {
        ...
        internal static List<CustomSortingMethod> SortPriorities = new List<CustomSortingMethod> { CustomSortingMethod.Rarity, CustomSortingMethod.Name };

        public static System.Collections.Generic.Dictionary<Rarity, int> RarityOrder = new System.Collections.Generic.Dictionary<Rarity, int> {
            { Rarity.Oddity, 5 },
            { Rarity.Exotic, 4 },
            { Rarity.Epic, 3 },
            { Rarity.Rare, 2 },
    ...
    harmony.PatchAll(typeof(GearDetailsWindowpatches));
    harmony.PatchAll(typeof(SortUpgradesPatch));
        ...
        internal void OnGearDetailsWindowOpen(GearDetailsWindow window)
        {
            ...
            window.StartCoroutine(DelayedButtonCreation(window));
        }
    ...
    public static class GearDetailsWindowpatches
    {
        ...
        public static List<GearUpgradeUI> ApplyFilters(List<GearUpgradeUI> upgradeUIs, FilterSettings filters)
        {
            ...
            foreach (var ui in upgradeUIs)
            {
                if (filters.FavoriteSetting == FavoriteFilter.ShowOnlyFavorited && !ui.Upgrade.Favorite) continue;
                if (filters.FavoriteSetting == FavoriteFilter.HideFavorited && ui.Upgrade.Favorite) continue;
                filtered.Add(ui);
            }
            return filtered;
        }
            ...
            try
            {
                ...
                if (currentSkinModeField != null)
                {
                    ...
                    if (UpgradeSortingPlugin.PreviousSkinMode.HasValue && UpgradeSortingPlugin.PreviousSkinMode.Value != currentSkinMode)
                    {
                        ...
                        if (UpgradeSortingPlugin.Instance != null && UpgradeSortingPlugin.Instance.FilterPanel != null)
                        {
    ...
    public static class SortUpgradesPatch
    {
        ...
        public static void Postfix(GearDetailsWindow __instance, GearDetailsWindow.SortingMethod method, bool resetScroll)
        {
            ...
            if (method == GearDetailsWindow.SortingMethod.Name)
            {
                try
                {
                    var upgradeUIsField = AccessTools.Field(typeof(GearDetailsWindow), "upgradeUIs") ??
                    ...
                    if (upgradeUIsField != null)
                    {
                        var upgradeUIs = (List<GearUpgradeUI>)upgradeUIsField.GetValue(__instance);
                        if (upgradeUIs != null && upgradeUIs.Count >= 2)
                        {
                            upgradeUIs.Sort(PropertyAwareNameComparison);
                        }
                    }
                }
                catch (Exception e)
